package com.user.User.aopService;
...
public class AOPService
{
    ...
    @Before("execution(* com.user.User.controller.UserRegistrationController.*(..))")
    public void logBeforeV1(JoinPoint joinPoint) {
        log.info("Initiating API : " + joinPoint.getSignature().getName() + " ");
    }

    //logs method After hitting API
    @After("execution(* com.user.User.controller.UserRegistrationController.*(..))")
    ...
    //ExceptionHandling
    @Around(value = "execution(* com.user.User.controller.UserRegistrationController.*(..))")
